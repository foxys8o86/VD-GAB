...
from models.gcn_bilstm import GCN_BiLstm
...
for fold_id in range(n_folds):
    ...
    if args.model == 'gcn_bilstm':
        model = GCN_BiLstm(in_features=loaders[0].dataset.num_features,
                           out_features=loaders[0].dataset.num_classes,
                           n_hidden=args.n_hidden,
                           filters=args.filters,
                           dropout=args.dropout,
                           adj_sq=args.adj_sq,
                           scale_identity=args.scale_identity).cuda()
    ...
    def train(train_loader):
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            if batch_idx % 10 == 0:
                ...
                correct = (predicted == data[4]).sum().item()
    ...
    def test(test_loader):
        ...
        for batch_idx, data in enumerate(test_loader):
            for i in range(len(data)):
                data[i] = data[i].cuda()
            output = model(data)
            loss = loss_fn(output.cuda(), data[4], reduction='sum')
            test_loss += loss.item()
            n_samples += len(output)
            count += 1
